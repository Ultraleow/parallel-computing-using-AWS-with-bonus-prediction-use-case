...
if selected == "For Individual":
    ...
    st.markdown("Please fill in all required information marked with * to proceed with further processing.")
    ...
    with col3:
        country = st.selectbox("Country You Work At*",
    ...
    with col4:
        state = st.selectbox("State You Work At*",
    ...
    with col5:
        yearsofexperience = st.number_input("Years of Working Experience*",
    ...
    if gender and Education and Race and country and company and state and title and yearsofexperience and\
        ...
        if st.button(label="Submit"): #on_click=form_callback())
            ...
            response = requests.post(f"http://34.200.183.255:5000/predict", json=inputs, verify=False)
            json_response = response.json()

            prediction = class_values[json_response.get("predictions")[0]]

            st.subheader(f"Your predicted salary is **{prediction}!**")
        else:
            st.stop()
    else:
        st.stop()
...
if selected == "Bulk Data Processing":
    ...
    st.markdown("Please fill in all required information marked with * to proceed with file submission.")
